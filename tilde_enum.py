...
def initialCheckUrl(url):
	...
	print '[-]  Testing with dummy file request %s://%s/%s.htm' % (u.scheme, u.netloc, not_there_string)
	...
	if response_code['dummy_code'] != 404:
		print '[!]  FALSE POSITIVE ALERT: We may have a problem determining real responses since we did not get a 404 back.'

	# Now that we have the "definitely not there" page, check for one that should be there
	print '[-]  Testing with user-submitted %s' % (url)
	...
	if response_code['user_code'] != 200:
		print '[!]  ERROR: We did not receive an HTTP response code 200 back. Please check URL.'
		sys.exit()
	else:
...
def readScanFile(file_name):
	...
	try:
		...
		print '[!]  [Error] Can\'t read the scanner file you entered.'
		sys.exit()
...
def searchFileForString(string, file):
	...
	try:
		...
		print '[!]  [Error] Can\'t read the wordlist file you entered.'
		sys.exit()
...
def main():
	...
	if not targets:
		print '[!]  Error: No FILE entries found in scanner output'
		sys.exit()
	...
	if args.v: print '[+]  HTTP Response Codes: %s' % response_code

	filename_matches = []

	# Start the URL requests to the server
	print '[-]  Now starting the web calls'
	...
	for file in targets:
		...
		if args.v: print '[+]  Searching for %s in word list' % filename
...
if args.v:
	print "[-]  Entering 'Verbose Mode'....brace yourself for additional information."
